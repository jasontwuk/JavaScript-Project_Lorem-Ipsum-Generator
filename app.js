...
loremForm.addEventListener('submit', function(e){
  ...
  // *** we get a string from amount, so need to turn it into a number
  const value = parseInt(amount.value);
  // console.log(typeof value);

  // *** for odd request, show one random paragraph
  if(isNaN(value) || value <= 0 || value > 9){
    const randomNumber = Math.floor(Math.random() * text.length);
    // console.log(randomNumber);

    // *** turn to html format and dispaly on screen
    loremText.innerHTML = `<p class="result">${text[randomNumber]}</p>`;
  } else {
    // *** get the asked number of paragraphs
